...
class CRM_Evenement_Page_InfoEvent extends CRM_Core_Page {
  ...
  public function run() {
    ...
    $breadCrumbPath = CRM_Utils_System::url('civicrm/event/participant',
      "id={$this->_id}&reset=1"
    );
    ...
    if (!empty($values['event']['is_online_registration'])) {
      if ($isEventOpenForRegistration == 1) {
        ...
        $url = CRM_Utils_System::url('civicrm/participation-event',
      ...
      if (CRM_Event_BAO_Event::checkRegistration($params)) {
        if ($noFullMsg == 'false') {
          if ($values['event']['allow_same_participant_emails']) {
            ...
            $registerUrl = CRM_Utils_System::url('civicrm/participation-event', "reset=1&id={$values['event']['id']}&cid=0" );
            ...
            $statusMessage = ts("Vous êtes déjà inscrits à cet événement. Si vous voulez changer votre inscription veuillez cliquer 
            
            <a href='%1'>modifier participation</a>", [1 => $registerUrl]);
